...
- `VISUAL_DOES_NOT_PROVE_HEADLINE` (hard) — the headline names a quantity (`"18 skills"`) larger than the composition's countable elements actually show (5 stage cards). Fix by making the visual show the number (e.g. `process.content.steps[].members`, rendered as chips on `stage_gate` and counted toward the claim) or by not claiming more than the slide shows.
...
```
Validates and rolls the findings into `<run-dir>/visual-qa.json`; a hard finding fails the run. Pass `--pptx` to compare the file being judged against `visual/render-provenance.json`: a digest mismatch (the PPTX or DeckSpec changed since `visual` last ran) is hard `VISUAL_QA_STALE_RENDER`; no provenance file at all (a run made before this existed) is risk `VISUAL_RENDER_PROVENANCE_UNKNOWN`. Without `--pptx` this check is skipped entirely — always pass it. A non-empty `backend.json.substitutedFonts` is folded in automatically as risk `RENDER_FONT_SUBSTITUTION`.
...
`release` additionally accepts `--visual-qa <path>` and `--accept-risk`: a hard visual finding always blocks; an unresolved risk finding blocks unless `--accept-risk` is passed, in which case the release status is `pass_with_warning` instead of `pass`. Pass `--run-dir <run-dir>` alongside `--visual-qa` so release can compare the PPTX being released against `visual/render-provenance.json`: releasing a file re-rendered after Visual QA last judged it is blocked, not silently shipped as if it had been re-checked.
