...
| — | Headline claims a visual must prove | done — `VISUAL_DOES_NOT_PROVE_HEADLINE` (hard, Core QA) checks a headline's numeric claim against the composition's actual countable elements; `process.content.steps[].members` lets a cluster-label composition (`stage_gate`) prove a claim it groups rather than shows one-to-one | `src/qa.ts`, `src/schema.ts` |
