...
| — | Headline claims a visual must prove | done — `VISUAL_DOES_NOT_PROVE_HEADLINE` (Core QA) checks a headline's numeric claim against the composition's actual countable elements; a heuristic-only mismatch is `risk`, an explicit `slide.visualProof: { kind: "count", value, collection }` contract violation is `hard`. `process.content.steps[].members` lets a cluster-label composition (`stage_gate`) prove a claim it groups rather than shows one-to-one | `src/qa.ts`, `src/schema.ts` |
| — | Visual QA judges the artifacts it produced, not a later re-render | done — `visual/render-provenance.json` digests the PPTX+DeckSpec that produced a render; `visual-qa --pptx` and `release --run-dir` both compare against it (`VISUAL_QA_STALE_RENDER` hard, `VISUAL_RENDER_PROVENANCE_UNKNOWN` risk); the `soffice` intermediate PDF is kept as `visual/deck.pdf` instead of discarded, and its embedded fonts are checked against the contracted heading/body pair (`RENDER_FONT_SUBSTITUTION`) | `src/visual.ts`, `src/visual-qa.ts`, `src/cli.ts` |
| §13 §14 | Token telemetry never reports a misleading zero | done — `resolveTranscript` picks whichever transcript in the project-slug directory actually covers the measurement window instead of merely the newest by mtime; `tokens.json.measurement` is `"unavailable"` (with `unavailableReason`, `null` per-slide ratios) rather than `0` when no turns are found; `record` refuses to merge an unavailable report | `src/tokens.ts`, `src/score.ts`, `src/cli.ts` |
