...
describe("P3 presentation style resolution", () => {
  ...
  it("gives each archetype a distinct grammar signature once palette is removed", () => {
    const signatures = presentationArchetypes.map((themeId) => JSON.stringify(resolvePresentationStyle(contractSchema.parse({ ...fixture, presentationStyle: themeId }), { projectDir: process.cwd() }).grammar));
