...
An organisation pack is selected with `contract.organization: { kind: "directory", path: "organizations/acme" }` and contains `template.pptx`, `brand.yaml`, and `template-map.json`. The template must be 16:9. The shared semantic renderer writes a scratch deck, then `pptx-automizer` applies native masters/layouts without an organisation-specific renderer fork.
